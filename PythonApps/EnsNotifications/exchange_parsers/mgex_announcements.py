...
def get_date(string_value):
    ...
    # Using dict to make format patterns scalable for future
    date_patterns = {
        'leading_zeros': {
            'regex': '(0|1)?\d{1}-(0|1|2|3)?\d{1}-(0|1|2)\d',
            'format': '%m-%d-%y'
        }
    }

    for each_pattern in date_patterns:
        compiled_pattern = re.compile(date_patterns[each_pattern]['regex'])
        match = compiled_pattern.search(string_value)
        if match:
            start_pos = match.span()[0]
            end_pos = match.span()[1]
            converted_string = datetime.strptime(match.string[start_pos:end_pos], date_patterns[each_pattern]['format'])
...
def set_values(notice_table_row):
    ...
    notice.eventPublishDate = get_date(notice_table_row.a.text)
